...
for k in data['reg_score'].item().keys():
    ...
    if 'linreg' in data.keys():
        if np.sum([len(x) for x in data['linreg'].item().values()]):
            linreg=True
    binset=['1','3']###### FIXME read in number of bins tested
    good_features='clust'
data.close()
...
binary_features=['clust','dist']
label_dict={'None':'Direct','corr':'+Correlation','dist':'+Distance','corr_dist':'+Both','clust':'cluster length'}

#### read feature importance, regression scores and measures of ending synapse weight
testset_scores,endwt_measures,feat_import=read_data(files,testset_scores,endwt_measures,feat_import)
...
#new stuff for linear regresion
if linreg:
    ...
    for regtype in ['test','train']:
        ...
        bar_plot_mean_score(means,np.unique(lin_test_df.numbins),stderr,'linear Regression: '+regtype)
...
if len(np.unique(test_features.other_feature))>1:
    ...
    order_dict={'None':0,'clust':1,'dist':2}
...
ax[0].set_ylim([0,0.7])
'''
bar_plot_mean_score(means,np.unique(test_features.numbins),stderr,pattern.split('.')[0])
...
print('## All Data - no adjacent, distance or cluster info ### ')
...
model=ols(base_model+'+'+binary_features[0]+'+'+binary_features[1],data=test_features).fit()
anova2way = sm.stats.anova_lm(model)
print(anova2way)
print(model.summary())
